...
def show_label(text: str, x: int, y: int, font_name: str ='Arial', color:tuple=(255, 255, 255)):
    '''
    ВЫВОДИТ НАДПИСЬ НА ИГРОВУЮ СЦЕНУ ВИНДОВ
...
killed_counter = Counter('Счетчик уничтоженных:',10,100)
        ...
        for monster in list_monsters:
            ...
            if monster.hp <= 0:
                ...
                if killed_counter.counter == 20:
                    ufos.add(Enemy('ufo.png', randint(1, 5), randint(1, SCREEN_SIZE[0]-SPRITE_SIZE), 0))
                    ufos.add(Enemy('ufo.png', randint(1, 5), randint(1, SCREEN_SIZE[0]-SPRITE_SIZE), 0))
